...
struct RadixDecimalCache {
    last: Option<RadixDecimalCacheEntry>,
}

struct RadixDecimalCacheEntry {
    radix: u32,
    digits: String,
    decimal: String,
}
...
fn parse_frontmatter(
    ...
    if let Some((relative_start, relative_end)) = invalid_yaml_indentation_tab_range(yaml_source) {
        return Err(FrontmatterError {
            message: "Invalid YAML frontmatter: Tabs are not allowed as indentation".to_owned(),
            range: Some(range_for(
                text,
                opening_end + relative_start,
                opening_end + relative_end,
    ...
    if let Some((relative_start, relative_end, misaligned)) =
        closed_flow_sequence_followup_range(yaml_source)
    {
        return Err(FrontmatterError {
            message: if misaligned {
                "Invalid YAML frontmatter: All sequence items must start at the same column"
                    .to_owned()
            } else {
                "Invalid YAML frontmatter: Sequence item without - indicator".to_owned()
            },
            range: Some(range_for(
                text,
                opening_end + relative_start,
    ...
    if let Some((relative_start, relative_end, message)) =
        invalid_tagged_flow_property_value_range(yaml_source)
    {
        return Err(FrontmatterError {
            message: format!("Invalid YAML frontmatter: {message}"),
            range: Some(range_for(
                text,
                opening_end + relative_start,
                opening_end + relative_end,
    ...
    if let Some((relative_start, relative_end)) = flow_block_mapping_value_range(yaml_source) {
        return Err(FrontmatterError {
            message: "Invalid YAML frontmatter: Block collections are not allowed within flow collections"
                .to_owned(),
            range: Some(range_for(
                text,
                opening_end + relative_start,
    ...
    if let Some((relative_start, relative_end, indicator)) =
        invalid_reserved_plain_indicator_range(yaml_source)
    {
        return Err(FrontmatterError {
            message: format!(
                "Invalid YAML frontmatter: Plain value cannot start with reserved character {indicator}"
            ),
            range: Some(range_for(
                text,
                opening_end + relative_start,
    ...
    let tab_safe_yaml = normalize_permitted_yaml_tabs(&normalized_yaml);
    let (plain_safe_yaml, plain_continuation_marker) =
        mask_plain_scalar_continuation_indicators(&tab_safe_yaml);
...
fn multiple_block_node_property_range(
    ...
    for (line_index, line) in spans.iter().enumerate() {
        ...
        for candidate in spans.iter().skip(line_index + 1) {
            ...
            let property_node = child_trimmed;
            let leading = child_body.len().saturating_sub(child_trimmed.len());
            let property_start = candidate.start + leading;
            let child_colon = mapping_key_colon(property_node);
            let property_source = if let Some(child_colon) = child_colon {
                let key_source = property_node[..child_colon].trim_end_matches([' ', '\t']);
    ...
    for (line_index, line) in spans.iter().enumerate() {
        ...
        let trimmed = body.trim_start_matches([' ', '\t']);
        let indent = body.len() - trimmed.len();
        let (initial, initial_offset) = if let Some(item) = trimmed.strip_prefix("- ") {
            (item, 2)
        } else if trimmed == "-" {
            ("", 1)
        } else {
        ...
        let mut collection_property = false;
        if !initial.is_empty() {
            let (remainder, anchor, tag) = split_node_properties(initial);
            if (anchor.is_none() && tag.is_none())
            ...
            let property_spans = match property {
                NodePropertyKind::Tag => node_tag_spans(initial),
                NodePropertyKind::Anchor => node_anchor_spans(initial),
            };
            for (property_start, property_end) in property_spans {
                if property_count > 0 {
                    let start = line.start + indent + initial_offset;
                    return Some((start + property_start, start + property_end));
                }
                property_count += 1;
            }
            collection_property =
                tag.is_some_and(|tag| matches!(tag, "map" | "seq" | "set" | "omap" | "pairs"));
        }
        for candidate in spans.iter().skip(line_index + 1) {
            let child_body = &source[candidate.start..candidate.content_end];
            let child_trimmed = child_body.trim_start_matches([' ', '\t']);
            ...
            let child_indent = child_body.len() - child_trimmed.len();
            if child_indent <= indent || child_trimmed.starts_with(['?', '-', ':']) {
                break;
            }
            let child_colon = mapping_key_colon(child_trimmed);
            let property_source = if let Some(colon) = child_colon {
                &child_trimmed[..colon]
            } else {
                child_trimmed
            };
            let property_source = property_source.trim_end_matches([' ', '\t']);
            let (remainder, anchor, tag) = split_node_properties(property_source);
            let property_spans = match property {
                NodePropertyKind::Tag => node_tag_spans(property_source),
                NodePropertyKind::Anchor => node_anchor_spans(property_source),
            };
            let start = candidate.start + child_indent;
            if child_colon.is_some() && collection_property && anchor.is_none() {
                break;
            }
            if anchor.is_none() && tag.is_none() {
                break;
            ...
            if tag.is_some_and(|tag| matches!(tag, "map" | "seq" | "set" | "omap" | "pairs"))
                && (remainder.is_empty() || remainder.starts_with('#'))
            {
                collection_property = true;
            }
            if child_colon.is_some() || (!remainder.is_empty() && !remainder.starts_with('#')) {
                break;
            }
        }
    }
...
fn internal_scalar_tagged_string_key(tag: &str, value: &serde_yaml::Value) -> Option<String> {
    ...
    match tag {
        ...
        "!oseq" | "!omap-value" | "!oset-value" | "!oomap" | "!opairs" => Some(source),
        _ => None,
    }
}
...
fn parsed_string_mapping_key(source: &str, tag_name: Option<&str>) -> Option<String> {
    ...
    if yaml_package_plain_binary_is_string(source) {
        return Some(scalar_lexical_source(source).trim().to_owned());
    }
    let (masked, marker) = mask_literal_nel(source);
    let parsed = serde_yaml::from_str::<String>(&masked).ok();
...
fn invalid_yaml_indentation_tab_range(source: &str) -> Option<(usize, usize)> {
    ...
    line_spans(source).into_iter().find_map(|line| {
        if sorted_range_contains(&scalar_ranges, line.start) {
            return None;
        }
        let body = &source[line.start..line.content_end];
        let leading_spaces = body.bytes().take_while(|byte| *byte == b' ').count();
        if body.as_bytes().get(leading_spaces) == Some(&b'\t') {
            let content = body[leading_spaces..].trim_start_matches([' ', '\t']);
            if !content.is_empty() && !content.starts_with('#') {
                let start = line.start + leading_spaces;
                return Some((start, start + 1));
            }
        }
        let after_indent = &body[leading_spaces..];
        after_indent.strip_prefix('-').and_then(|rest| {
            let separator_length = rest
                .bytes()
                .take_while(|byte| matches!(*byte, b' ' | b'\t'))
                .count();
            let separator = &rest[..separator_length];
            let tab = separator.find('\t')?;
            let content = &rest[separator_length..];
            (content.starts_with(['&', '!'])
                || content.strip_prefix(['?', ':']).is_some_and(|rest| {
                    rest.is_empty() || rest.chars().next().is_some_and(char::is_whitespace)
                }))
            .then(|| {
                let start = line.start + leading_spaces + 1 + tab;
                (start, start + 1)
            })
        })
    })
...
fn normalize_permitted_yaml_tabs(source: &str) -> String {
    let mut replacements = Vec::new();
    for line in line_spans(source) {
        let body = &source[line.start..line.content_end];
        let leading_spaces = body.bytes().take_while(|byte| *byte == b' ').count();
        let after_spaces = &body[leading_spaces..];
        if after_spaces.starts_with('\t')
            && after_spaces
                .trim_start_matches([' ', '\t'])
                .starts_with('#')
        {
            let tab_count = after_spaces
                .bytes()
                .take_while(|byte| *byte == b'\t')
                .count();
            replacements.extend((0..tab_count).map(|offset| line.start + leading_spaces + offset));
        }
            ...
            replacements.extend(
                after_dash
                    ...
                    .filter_map(|(offset, byte)| {
                        (byte == b'\t').then_some(line.start + leading_spaces + 1 + offset)
                    }),
            );
        }
    }
    ...
    for offset in replacements {
        normalized.replace_range(offset..offset + 1, " ");
    }
    normalized
}
    ...
    for (offset, _) in source.match_indices('*') {
        ...
        let anchor_defined = yaml_anchor_occurrences_in_ranges(source, &[(0, offset)])
            ...
            .any(|(anchor_offset, anchor_name)| {
                anchor_name == name
                    && (!sorted_range_contains(&excluded_ranges, anchor_offset)
                        || yaml_node_property_position(source, anchor_offset))
            });
        if !anchor_defined {
...
fn invalid_reserved_plain_indicator_range(source: &str) -> Option<(usize, usize, char)> {
    let mut excluded_ranges = block_scalar_body_ranges(source);
    excluded_ranges.extend(multiline_quoted_scalar_ranges(source));
    normalize_ranges(&mut excluded_ranges);
    for (offset, indicator) in source
        .char_indices()
        .filter(|(_, character)| matches!(character, '@' | '`'))
    {
        ...
        return Some((offset, offset + indicator.len_utf8(), indicator));
    }
    None
}
...
fn compact_nested_mapping_range(source: &str) -> Option<(usize, usize)> {
    ...
    for (line_index, span) in spans.iter().enumerate() {
        ...
        let mapping_indent = indent + usize::from(item.is_some()) * 2;
        let mapping_source = item.unwrap_or(trimmed);
        if matches!(mapping_source.chars().next(), Some('{' | '[')) {
            continue;
        }
        ...
        let after_colon = &mapping_source[colon + 1..];
        let leading = after_colon.len() - after_colon.trim_start().len();
        let value = after_colon.trim_start();
        let value_start =
            span.start + indent + usize::from(item.is_some()) * 2 + colon + 1 + leading;
        let (property_remainder, property_anchor, property_tag) = split_node_properties(value);
        if (property_anchor.is_some() || property_tag.is_some())
            && !matches!(
                property_remainder.chars().next(),
                Some('{' | '[' | '|' | '>')
            )
            && mapping_key_colon(property_remainder).is_some()
        {
            return Some((
                value_start,
                one_character_end(source, value_start, source.len()),
            ));
        }
        if value.is_empty()
            || value.starts_with(['#', '"', '\'', '[', '{', '|', '>', '&', '!', '*'])
        ...
        let nested_mapping = spans.iter().skip(line_index + 1).find_map(|candidate| {
            ...
            Some(
                nested_indent > mapping_indent
                    && nested_colon.is_some()
                    && !literal_nel_continuation,
            )
        });
...
fn closed_flow_sequence_followup_range(source: &str) -> Option<(usize, usize, bool)> {
    let spans = line_spans(source);
    let mut excluded_ranges = block_scalar_body_ranges(source);
    excluded_ranges.extend(multiline_quoted_scalar_ranges(source));
    normalize_ranges(&mut excluded_ranges);
    for (line_index, line) in spans.iter().enumerate() {
        ...
        let (remainder, _, _) = split_node_properties(item);
        if !matches!(remainder.chars().next(), Some('{' | '[')) {
            continue;
        }
        let flow_start = line.start + body.len() - trimmed.len() + 2 + item.len() - remainder.len();
        if flow_value_end(source, flow_start) > line.content_end {
            continue;
        }
        for candidate in spans.iter().skip(line_index + 1) {
            let candidate_body = &source[candidate.start..candidate.content_end];
            ...
            let misaligned = candidate_trimmed
                .strip_prefix('-')
                .is_some_and(|rest| rest.is_empty() || rest.starts_with([' ', '\t']));
            let start = candidate.start + if misaligned { candidate_indent } else { 0 };
            return Some((
                start,
...
fn flow_block_mapping_value_range(source: &str) -> Option<(usize, usize)> {
        ...
        .filter(|(start, _)| source[*start..].starts_with('{'))
    {
        for (entry_start, entry_end) in top_level_flow_entries(source, flow_start, flow_end) {
            let Some(colon) = flow_top_level_mapping_colon(source, entry_start, entry_end) else {
                continue;
            };
            let value_start = skip_flow_space_and_comments(source, colon + 1, entry_end);
            let value_source = source[value_start..entry_end].trim_end();
            ...
            if remainder.trim_start().starts_with('#') {
                for line in line_spans(source)
                    .into_iter()
                    .filter(|line| value_start < line.start && line.start < entry_end)
                {
                    let body = &source[line.start..line.content_end];
                    let trimmed = body.trim_start_matches([' ', '\t']);
                    if trimmed.is_empty() || trimmed.starts_with('#') {
                    ...
                    if mapping_key_colon(trimmed).is_some() {
                        let start = line.start + body.len().saturating_sub(trimmed.len());
                        let syntax = trimmed.trim_end_matches([' ', '\t', '}', ']']);
                        return Some((start, start + syntax.len()));
                    }
                    break;
                }
            }
            let start = value_start + value_source.len().saturating_sub(remainder.len());
            return Some((start, start + remainder.trim_end().len()));
        }
    }
    None
}

fn invalid_tagged_flow_property_value_range(source: &str) -> Option<(usize, usize, &'static str)> {
    for (flow_start, flow_end) in flow_collection_ranges(source)
        .into_iter()
        .filter(|(start, _)| source[*start..].starts_with('{'))
    {
        for (entry_start, entry_end) in top_level_flow_entries(source, flow_start, flow_end) {
            ...
            let remainder_start = value_start + value_source.len().saturating_sub(remainder.len());
            if let Some(after_question) = remainder.strip_prefix('?')
                && after_question
                    .chars()
                    .next()
                    .is_some_and(char::is_whitespace)
            {
                ...
                let start = remainder_start + 1 + leading;
                return Some((
                    start,
                    one_character_end(source, start, entry_end),
                    "Missing , between flow map items",
                ));
            }
            if remainder.starts_with(':') {
                return Some((
                    remainder_start,
                    one_character_end(source, remainder_start, entry_end),
...
fn yaml_node_property_prefix_start(prefix: &str) -> usize {
    let sequence_start = prefix
        .trim_start()
        .strip_prefix("- ")
        .map(|rest| prefix.len() - rest.len())
        .unwrap_or(0);
    let mut structural_start = 0;
    let mut quote = None;
    let mut escaped = false;
    ...
    while let Some((index, character)) = characters.next() {
        ...
        match character {
            ...
            '"' | '\'' => quote = Some(character),
            '[' | '{' | ',' | ':' | '?' => structural_start = index + 1,
            _ => {}
        }
    }
    sequence_start.max(structural_start)
}
    ...
    for (line_index, span) in spans.iter().enumerate() {
        ...
        for tag in ["!!timestamp", "!<tag:yaml.org,2002:timestamp>"] {
            ...
            while let Some(relative) = line[search_start..].find(tag) {
                ...
                let property_start = yaml_node_property_prefix_start(prefix);
                ...
                let timestamp_mapping_key = timestamp_mapping_key_range_at(
                    source,
                    &spans,
                    line_index,
                    line,
                ...
                let sequence_property_start = prefix
                    .trim_start()
                    .strip_prefix("- ")
                    .map(|rest| prefix.len() - rest.len());
                let node_property_position =
                    follows_anchor_property && sequence_property_start == Some(property_start);
                if (sorted_range_contains(&excluded_ranges, absolute) && !node_property_position)
                    || yaml_comment_start(prefix).is_some()
                    || previous.is_some_and(|character| {
                        !matches!(character, '[' | '{' | ',' | ':' | '?' | '-')
                    }) && !follows_anchor_property
                    || (previous.is_none()
                        && !follows_deferred_node
                        && timestamp_mapping_key.is_none())
                {
                ...
                let lexical = if let Some((key_start, key_end)) = timestamp_mapping_key {
...
fn timestamp_mapping_key_range_at(
    source: &str,
    spans: &[LineSpan],
    line_index: usize,
    line: &str,
    ...
    let property_start = prefix
        ...
        .unwrap_or(0);
    if property_start > 0 && prefix[..property_start].ends_with(':') {
        return None;
    }
    let property_source = line[property_start..].trim_start();
    let mut node_start = property_start + line[property_start..].len() - property_source.len();
    let node_source = if let Some(source) = property_source.strip_prefix("- ") {
        node_start += 2;
        source
    } else {
        property_source
    };
    let relative_tag = tag_offset.checked_sub(node_start)?;
    let key_source = mapping_key_colon(node_source)
        .filter(|colon| relative_tag + tag_len <= *colon)
        .map(|colon| node_source[..colon].trim_end());
    if let Some(key_source) = key_source {
        let (plain_key_source, relative_start, _, key_tag) =
            multiline_mapping_key_parts(key_source)?;
        if key_tag != Some("timestamp") {
            return None;
        }
        let start = spans[line_index].start + node_start + relative_start;
        return Some((start, start + scalar_lexical_source(plain_key_source).len()));
    }

    let trimmed = line.trim_start_matches([' ', '\t']);
    let indent = line.len() - trimmed.len();
    let explicit_indent = if trimmed.strip_prefix("? ").is_some() || trimmed == "?" {
        Some(indent)
    ...
    let has_explicit_value = spans.iter().skip(line_index + 1).find_map(|candidate| {
        let body = &source[candidate.start..candidate.content_end];
        let candidate_trimmed = body.trim_start_matches([' ', '\t']);
        if candidate_trimmed.is_empty() || candidate_trimmed.starts_with('#') {
            return None;
        }
        let candidate_indent = body.len() - candidate_trimmed.len();
        if candidate_indent < explicit_indent {
            return Some(false);
        }
        (candidate_indent == explicit_indent).then(|| {
    ...
    let (plain_key_source, relative_start, _, key_tag) = multiline_mapping_key_parts(node_source)?;
    if key_tag != Some("timestamp") {
        return None;
    }
    let start = spans[line_index].start + node_start + relative_start;
...
fn rewrite_standard_sets_for_serde(source: &str) -> String {
    ...
    for (tag_start, tag_end) in occurrences {
        ...
        let mut deferred_anchor = None;
        let mut deferred_null_member = None;
        for candidate in spans.iter().skip(line_index + 1) {
            let body = &source[candidate.start..candidate.content_end];
            let trimmed = body.trim_start_matches([' ', '\t']);
            if trimmed.is_empty() || trimmed.starts_with('#') {
            ...
            if let Some(rest) = trimmed.strip_prefix(':').filter(|rest| {
                (rest.is_empty() || rest.chars().next().is_some_and(char::is_whitespace))
                    && semantic_null_deferred_set_property(rest.trim_start())
            }) {
                let _ = rest;
                deferred_null_member = Some((candidate, indent));
                break;
            }
            let (property_remainder, property_anchor, property_tag) =
                split_node_properties(trimmed);
            if (property_remainder.is_empty() || property_remainder.starts_with('#'))
                && (property_anchor.is_some() || property_tag.is_some())
            {
                if let Some(anchor) = property_anchor
                    && let Some(relative) = trimmed.find(&format!("&{anchor}"))
                {
                    let start = candidate.start + indent + relative;
                    deferred_anchor = Some((start, start + anchor.len() + 1, anchor.to_owned()));
                }
                continue;
            }
            break;
        }
        if let Some((member, member_indent)) = deferred_null_member {
            let replacement = deferred_anchor.as_ref().map_or_else(
                || "!oset".to_owned(),
                |(_, _, anchor)| format!("&{anchor} !oset"),
            );
            edits.push((tag_start, tag_end, replacement));
            if let Some((anchor_start, anchor_end, _)) = deferred_anchor {
                edits.push((
                    anchor_start,
                    anchor_end,
                    " ".repeat(anchor_end - anchor_start),
                ));
            }
            edits.push((
                member.start + member_indent,
                member.content_end,
                "- { null: null }".to_owned(),
            ));
            continue;
            ...
            .take_while(|(_, candidate)| candidate.start < block_end)
        {
            ...
            if indent == member_indent
                && trimmed
                    .strip_prefix(':')
                    .is_some_and(|rest| semantic_null_set_member(rest.trim_start()))
            {
                edits.push((
...
fn normalize_tight_flow_plain_keys(source: &str) -> String {
    ...
    for (flow_start, flow_end) in flow_collection_ranges(source) {
        ...
        for (entry_start, entry_end) in top_level_flow_entries(source, flow_start, flow_end) {
            ...
            let tight_indicator_start = entry
                .chars()
                .next()
                .filter(|first| matches!(first, '?' | ':'))
                .is_some_and(|first| {
                    entry[first.len_utf8()..]
                        .chars()
                        .next()
                        .is_some_and(|next| !next.is_whitespace())
                });
            if (entry.contains(':') || tight_indicator_start)
        ...
        match character {
            ...
            ':' => {
                ...
                if structural_colon && flow_stack.last() == Some(&'{') {
                    ...
                    let tight_indicator_start = key_source
                        .chars()
                        .next()
                        .filter(|first| matches!(first, '?' | ':'))
                        .is_some_and(|first| {
                            key_source[first.len_utf8()..]
                                .chars()
                                .next()
                                .is_some_and(|next| !next.is_whitespace())
                        });
                    if (key_source.ends_with(':') || tight_indicator_start)
                        && !key_source.is_empty()
...
fn mask_invalid_standard_scalar_tags(source: &str) -> String {
    ...
    for (line_index, span) in spans.iter().enumerate() {
        ...
        for (tag, replacement) in [
            ...
            ("!!seq", "!oseq"),
            ("!<tag:yaml.org,2002:seq>", "!oseq"),
            ("!!map", "!omap-value"),
            ("!<tag:yaml.org,2002:map>", "!omap-value"),
            ("!!set", "!oset-value"),
            ("!<tag:yaml.org,2002:set>", "!oset-value"),
            ("!!omap", "!oomap"),
            ("!<tag:yaml.org,2002:omap>", "!oomap"),
            ("!!pairs", "!opairs"),
            ("!<tag:yaml.org,2002:pairs>", "!opairs"),
        ] {
            let mut search_start = 0;
            while let Some(relative) = line[search_start..].find(tag) {
                let offset = search_start + relative;
                ...
                let (property_remainder, anchor_name, _) = split_node_properties(property_source);
                let follows_anchor_property =
                ...
                let after_tag_source = &line[offset + tag.len()..];
                let after_tag = after_tag_source.trim_start();
                let (key_remainder, _, _) = split_node_properties(after_tag);
                let tagged_mapping_key = previous
                    .is_none_or(|character| matches!(character, '-' | '{' | ',' | '?'))
                    && !matches!(key_remainder.chars().next(), Some('{' | '['))
                    && mapping_key_colon(key_remainder).is_some();
                let collection_scalar_tag = matches!(
                    replacement,
                    "!oseq" | "!omap-value" | "!oset-value" | "!oomap" | "!opairs"
                );
                if (collection_scalar_tag && !tagged_mapping_key)
                    || quoted
                    || yaml_comment_start(prefix).is_some()
                    || previous.is_some_and(|character| {
                        !matches!(character, '[' | '{' | ',' | ':' | '?' | '-')
                ...
                if tagged_mapping_key && let Some(colon) = mapping_key_colon(key_remainder) {
                    let lexical = key_remainder[..colon].trim_end_matches([' ', '\t']);
                    let tag_name = split_standard_tag_property(&line[offset..])
                        .map(|(tag_name, _)| tag_name)
                        .unwrap_or_default();
                    if !matches!(
                        lexical.chars().next(),
                        Some('"' | '\'' | '{' | '[' | '|' | '>')
                    ) && standard_tagged_plain_key_is_string(tag_name, lexical)
                    {
                        let after_tag_leading = after_tag_source.len() - after_tag.len();
                        let remainder_offset = after_tag.len() - key_remainder.len();
                        let lexical_start =
                            span.start + offset + tag.len() + after_tag_leading + remainder_offset;
                        let lexical_end = lexical_start + lexical.len();
                        let mut quoted_lexical =
                            serde_json::to_string(lexical).expect("YAML key text is JSON-safe");
                        if lexical.is_empty()
                            && (lexical_start == span.start + offset + tag.len()
                                || !source[..lexical_start]
                                    .chars()
                                    .next_back()
                                    .is_some_and(char::is_whitespace))
                        {
                            quoted_lexical.insert(0, ' ');
                        }
                        replacements.push((lexical_start, lexical_end, quoted_lexical));
                    }
                }
                search_start = offset + tag.len();
            }
...
fn mask_large_yaml_integers(source: &str) -> String {
    ...
    for span in &spans {
        ...
        for (offset, character) in syntax.char_indices() {
            ...
            let string_resolving_collection_tag = properties.split_whitespace().any(|property| {
                matches!(
                    property,
                    "!!seq"
            ...
            if string_resolving_collection_tag {
...
fn non_string_mapping_key_range(source: &str) -> Option<(usize, usize)> {
    ...
    let standard_set_member_indents = standard_set_ranges
        .iter()
        .filter_map(|(set_start, set_end)| {
            spans
                .iter()
                .filter(|line| *set_start <= line.start && line.start < *set_end)
                .filter_map(|line| {
                    let body = &source[line.start..line.content_end];
                    let trimmed = body.trim_start_matches([' ', '\t']);
                    trimmed
                ...
                .min()
                .map(|indent| (*set_start, *set_end, indent))
        })
        .collect::<Vec<_>>();
    ...
    for (line_index, line) in spans.iter().enumerate() {
        ...
        let direct_standard_set_member =
            standard_set_member_indents
                .iter()
                .any(|(start, end, member_indent)| {
                    *start <= line.start && line.start < *end && indent == *member_indent
                });
            ...
            && let Some(key_end) = explicit_mapping_value_start
        {
            ...
            let first_line = key_source.lines().next().unwrap_or(key_source).trim_start();
            let (first_remainder, first_anchor, first_tag) = split_node_properties(first_line);
            let direct_deferred_member = direct_standard_set_member
                && (first_remainder.is_empty() || first_remainder.starts_with('#'))
                && (first_anchor.is_some() || first_tag.is_some());
            let (set_member, _, _) = split_node_properties(key_source.trim_start());
            if !(direct_deferred_member
                || direct_standard_set_member
                    && matches!(set_member.chars().next(), Some('{' | '[')))
                && let Some(range) = non_string_key_source_range(key_source, key_start)
            {
                return Some(range);
            }
        }
        ...
        if mapping_source.strip_prefix('?').is_some_and(|rest| {
            ...
            if let Some(range) = tagged_flow_collection_key_range(candidate, key_start) {
                return Some(range);
            }
        ...
        let (item_remainder, _, _) = split_node_properties(mapping_source);
        let property_flow_colon = if matches!(item_remainder.chars().next(), Some('{' | '[')) {
            let flow_start = mapping_source.len().saturating_sub(item_remainder.len());
            let flow_end = flow_value_end(mapping_source, flow_start);
            let after_flow = &mapping_source[flow_end..];
            let leading = after_flow.len() - after_flow.trim_start().len();
            if after_flow.trim_start().starts_with(':') {
                Some(flow_end + leading)
            } else {
                continue;
            }
        } else {
        ...
        let Some(colon) = property_flow_colon.or_else(|| mapping_key_colon(mapping_source)) else {
            continue;
        };
        let key_source = mapping_source[..colon].trim_end_matches([' ', '\t']);
        if key_source.is_empty() {
            if item.is_none() {
                if mapping_source.strip_prefix(':').is_some_and(|value| {
                    (value.is_empty() || value.chars().next().is_some_and(char::is_whitespace))
                        && semantic_null_deferred_set_property(value.trim_start())
                }) && previous_line_has_deferred_standard_set(source, &spans, line_index)
                {
                    continue;
                }
        ...
        if let Some(range) = tagged_flow_collection_key_range(key_source, key_start) {
            return Some(range);
        }
        if let Some(range) = non_string_key_source_range(key_source, key_start) {
            return Some(range);
...
fn previous_line_has_deferred_standard_set(
    source: &str,
    spans: &[LineSpan],
    line_index: usize,
) -> bool {
    for candidate in spans[..line_index].iter().rev() {
        let body = &source[candidate.start..candidate.content_end];
        let trimmed = body.trim_start_matches([' ', '\t']);
        if trimmed.is_empty() || trimmed.starts_with('#') {
            continue;
        ...
        let (remainder, _, tag) = split_node_properties(value_source);
        if remainder.is_empty() || remainder.starts_with('#') {
            if tag == Some("set") {
                return true;
            }
            let (_, anchor, _) = split_node_properties(value_source);
            if anchor.is_some() {
                continue;
            }
        }
...
fn non_string_key_source_range(key_source: &str, key_start: usize) -> Option<(usize, usize)> {
    ...
    let yaml_12_leading_zero_integer = yaml_package_leading_zero_decimal(plain_key_source);
    let non_string = key_tag.is_some()
        || yaml_12_leading_zero_integer
        || !yaml_package_plain_binary_is_string(plain_key_source)
...
fn tagged_flow_collection_key_range(key_source: &str, key_start: usize) -> Option<(usize, usize)> {
    let (plain_key_source, relative_start, _, key_tag) = multiline_mapping_key_parts(key_source)?;
    key_tag?;
    let leading = plain_key_source.len() - plain_key_source.trim_start().len();
    let plain_trimmed = plain_key_source.trim_start();
    if !matches!(plain_trimmed.chars().next(), Some('{' | '[')) {
        return None;
    }
    let start = key_start + relative_start + leading;
    Some((start, start + flow_value_end(plain_trimmed, 0)))
}

fn standard_tagged_plain_key_is_string(tag_name: &str, source: &str) -> bool {
    match tag_name {
        "str" | "seq" | "map" | "set" | "omap" | "pairs" => true,
        "int" => {
            yaml_schema_integer_string(source) || canonical_set_integer(source, true).is_none()
        }
        "float" => {
            plain_nonfinite_kind(source).is_none()
                && (canonical_set_integer(source, true).is_some() || source.parse::<f64>().is_err())
        }
        "bool" => !matches!(source.to_ascii_lowercase().as_str(), "true" | "false"),
        "null" => !matches!(source, "" | "~" | "null" | "Null" | "NULL"),
        _ => false,
    }
}
...
fn standard_tagged_key_has_string_semantic(source: &str, tag_name: &str) -> bool {
    ...
    if tag_name == "float" && plain_nonfinite_kind(scalar_lexical_source(source).trim()).is_some() {
        return false;
    }
...
fn yaml_package_leading_zero_decimal(source: &str) -> bool {
    let plain = scalar_lexical_source(source).trim();
...
fn yaml_package_plain_binary_is_string(source: &str) -> bool {
    let plain = scalar_lexical_source(source).trim();
    let unsigned = plain.strip_prefix(['+', '-']).unwrap_or(plain);
...
fn semantic_mapping_key_identity(source: &str, tag_name: Option<&str>, fallback: &str) -> String {
    if tag_name.is_none() && yaml_package_leading_zero_decimal(source) {
        let plain = scalar_lexical_source(source).trim();
        let canonical = canonical_set_integer(plain, false).unwrap_or_else(|| plain.to_owned());
        return format!("number:{canonical}");
    }
    if tag_name.is_none() && yaml_package_plain_binary_is_string(source) {
        return format!(
            "string:{}",
            serde_json::to_string(scalar_lexical_source(source).trim())
                .unwrap_or_else(|_| fallback.to_owned())
        );
    }
    let semantic_key = semantic_mapping_key_value(source, tag_name)
    ...
    format!(
        ...
        serde_json::to_string(&semantic_key).unwrap_or_else(|_| fallback.to_owned())
    )
}
    ...
    for (offset, character) in source.char_indices() {
        ...
        match character {
            ...
            '{' => {
                ...
                let direct_standard_set = standard_flow_set_ranges
                    .iter()
                    .any(|(set_start, _)| *set_start == offset);
                for (entry_start, entry_end) in top_level_flow_entries(source, offset, end) {
                    ...
                    if !direct_standard_set
                        && matches!(key_source.trim_start().chars().next(), Some('[' | '{'))
                    {
                        let start = key_start + leading;
                        return Some((start, key_start + flow_value_end(key_source, leading)));
                    }
                    if !direct_standard_set
                        && let Some(range) = tagged_flow_collection_key_range(key_source, key_start)
                    {
                        return Some(range);
                    }
                    if direct_standard_set {
...
fn non_null_standard_set_value_range(source: &str) -> Option<(usize, usize)> {
    ...
    for (tag_start, tag_end) in standard_set_tag_occurrences(source) {
        ...
        if let Some(value) = after_tag.strip_prefix(':')
            && (value.is_empty() || value.chars().next().is_some_and(char::is_whitespace))
            && follows_deferred_block_node(source, &spans, line_index)
            && !semantic_null_deferred_set_property(value.trim_start())
        {
        ...
        if remainder.starts_with('{') {
            ...
            for (entry_start, entry_end) in top_level_flow_entries(source, flow_start, flow_end) {
                ...
                let mapping = entry
                    ...
                    .map(str::trim_start)
                    .unwrap_or(entry);
                let (member, _, _) = split_node_properties(mapping);
                let collection_colon = matches!(member.chars().next(), Some('{' | '['))
                    .then(|| mapping.len().saturating_sub(member.len()))
                    .map(|start| flow_value_end(mapping, start))
                    .filter(|end| mapping[*end..].trim_start().starts_with(':'))
                    .map(|end| end + mapping[end..].len() - mapping[end..].trim_start().len());
                let colon = if matches!(member.chars().next(), Some('{' | '[')) {
                    collection_colon
                } else {
                    mapping_key_colon(mapping)
                };
                if let Some(colon) = colon
                    && (mapping[colon + 1..].trim().is_empty()
                        && source[entry_end..flow_end].trim_start().starts_with('#')
        ...
        let parent_indent = line.len() - line.trim_start_matches([' ', '\t']).len();
        {
            for (candidate_index, candidate) in spans.iter().enumerate().skip(line_index + 1) {
                let body = &source[candidate.start..candidate.content_end];
                let trimmed = body.trim_start_matches([' ', '\t']);
                if trimmed.is_empty() || trimmed.starts_with('#') {
                    continue;
                ...
                if let Some(value) = trimmed.strip_prefix(':')
                    && (value.is_empty() || value.chars().next().is_some_and(char::is_whitespace))
                {
                    let empty_member_followed_by_comment = value.trim().is_empty()
                        && spans
                            .iter()
                            .skip(candidate_index + 1)
                            .find_map(|following| {
                                let following_body =
                                    &source[following.start..following.content_end];
                                let following_trimmed =
                                    following_body.trim_start_matches([' ', '\t']);
                                (!following_trimmed.is_empty())
                                    .then_some(following_trimmed.starts_with('#'))
                            })
                            == Some(true);
                    if empty_member_followed_by_comment
                        || !semantic_null_deferred_set_property(value.trim_start())
                    {
                        return Some((tag_start, tag_end));
                    }
        ...
        let member_indent = spans
            .iter()
            .filter(|candidate| span.end <= candidate.start && candidate.start < block_end)
            ...
            .filter(|candidate| span.end <= candidate.start && candidate.start < block_end)
        {
            ...
            let trimmed = body.trim_start_matches([' ', '\t']);
            let indent = body.len() - trimmed.len();
            if Some(indent) != member_indent {
                continue;
            }
            if let Some(value) = trimmed.strip_prefix(':')
                && (value.is_empty() || value.chars().next().is_some_and(char::is_whitespace))
                && !semantic_null_set_member(value.trim_start())
            {
                return Some((tag_start, tag_end));
            }
        }
...
fn semantic_null_set_member(member: &str) -> bool {
    ...
    if syntax.is_empty() {
        return member.trim().is_empty();
    }
    let (remainder, _, tag_name) = split_node_properties(syntax);
    if tag_name.is_some() {
        return false;
    }
    matches!(remainder, "" | "~" | "null" | "Null" | "NULL")
}

fn semantic_null_deferred_set_property(member: &str) -> bool {
    yaml_comment_start(member).is_none() && semantic_null_set_member(member)
}
...
fn decimal_with_signed_delta(value: &str, delta: i128) -> Option<String> {
    if delta == 0 {
    ...
    if delta > 0 {
        ...
        for digit in digits.iter_mut().rev() {
            let sum = u128::from(*digit - b'0') + carry % 10;
        ...
        let mut borrow = 0_u8;
        for digit in digits.iter_mut().rev() {
            let subtraction = (remaining % 10) as u8 + borrow;
            remaining /= 10;
            let current = *digit - b'0';
            if current >= subtraction {
                *digit = b'0' + current - subtraction;
                borrow = 0;
            } else {
                *digit = b'0' + current + 10 - subtraction;
                borrow = 1;
            }
        }
        if remaining > 0 || borrow > 0 {
            return None;
        }
        let leading = digits
            .iter()
...
fn radix_suffix_value(digits: &str, radix: u32) -> Option<u64> {
    digits.chars().try_fold(0_u64, |value, character| {
        value
...
fn cache_radix_decimal(radix: u32, digits: String, decimal: String) {
    if digits.len().saturating_add(decimal.len()) > MAX_RADIX_DECIMAL_CACHE_BYTES {
        return;
    }
    RADIX_DECIMAL_CACHE.with(|cache| {
        cache.borrow_mut().last = Some(RadixDecimalCacheEntry {
            radix,
            digits,
            decimal,
        });
...
fn decimal_from_radix(digits: &str, radix: u32) -> Option<String> {
    let cache_key = (digits.len() >= 1024).then(|| digits.to_owned());
    if let Some(value) = cache_key.as_ref().and_then(|key| {
        RADIX_DECIMAL_CACHE.with(|cache| {
            let cache = cache.borrow();
            let previous = cache.last.as_ref()?;
            if previous.radix != radix || previous.digits.len() != key.len() {
                return None;
            }
            if previous.digits == *key {
                return Some(previous.decimal.clone());
            }
            ...
            let suffix_length = key.len() - changed_at;
            let maximum_suffix = match radix {
                16 => 15,
                8 => 20,
                _ => 0,
            };
            if suffix_length > maximum_suffix {
                return None;
            }
            let old_suffix = radix_suffix_value(&previous.digits[changed_at..], radix)?;
            let new_suffix = radix_suffix_value(&key[changed_at..], radix)?;
            decimal_with_signed_delta(
                &previous.decimal,
                i128::from(new_suffix) - i128::from(old_suffix),
            )
        })
    }) {
        if let Some(key) = cache_key {
            cache_radix_decimal(radix, key, value.clone());
        }
        return Some(value);
    }
...
fn duplicate_mapping_key_range_with_mode(
    ...
    for (line_index, line) in spans.iter().enumerate() {
        ...
        if let Some(_explicit_key) = mapping_source.strip_prefix('?').and_then(|source| {
            ...
            let semantic_identity = semantic_mapping_key_identity(scalar, key_tag, &key);
            ...
            let scope = containers
                ...
                .collect::<Vec<_>>();
            if let Some(previous_tagged) = keys.insert((scope, semantic_identity), semantic_tagged)
            {
        ...
        let semantic_identity = semantic_mapping_key_identity(plain_key_source, key_tag, &key);
        let semantic_tagged = key_tag
            .is_some_and(|tag| standard_tagged_key_has_string_semantic(plain_key_source, tag));
        let scope = containers
            .iter()
        ...
        if let Some(previous_tagged) = keys.insert((scope, semantic_identity), semantic_tagged) {
            ...
            if plain_key_source.is_empty() {
                let mapping_offset = body
                    ...
                    .unwrap_or_else(|| body.len().saturating_sub(mapping_source.len()));
                let marker = line.start + mapping_offset + colon;
                return Some((marker, one_character_end(source, marker, line.content_end)));
            }
...
fn restored_standard_set_source(source: &str) -> String {
    if let Some(first) = line_spans(source).first().copied() {
        let body = &source[first.start..first.content_end];
        let trimmed = body.trim_start_matches([' ', '\t']);
        if let Some(value) = trimmed.strip_prefix(':')
            && (value.is_empty() || value.chars().next().is_some_and(char::is_whitespace))
            && semantic_null_deferred_set_property(value.trim_start())
        {
            let end = if value.trim().is_empty() {
                first.content_end
            } else {
                first.end
            };
            return source[..end].to_owned();
        }
    }
    let block_scalar_ranges = block_scalar_body_ranges(source);
    ...
    let terminal_empty_explicit_null = last.is_some_and(|body| {
        body.strip_prefix(':')
    ...
    } else if terminal_empty_explicit_null {
        source
            .strip_suffix("\r\n")
...
fn standard_tag_semantic(tag_name: &str, source_value: &str, existing: Value) -> Value {
    ...
    match tag_name {
        ...
        "set" => existing
            ...
            .unwrap_or(existing),
        "map" => {
            if !matches!(existing, Value::Array(_) | Value::Object(_))
                || existing
                    .as_object()
                    .is_some_and(|value| value.contains_key(EXACT_INTEGER_KEY))
            {
                return Value::String(scalar_text());
            }
...
fn direct_standard_set_member_sources(source: &str, expected_members: usize) -> Vec<String> {
    if source.trim_start().starts_with('{') {
        ...
        return top_level_flow_entries(source, flow_start, flow_end)
            .into_iter()
            .filter_map(|(start, end)| {
                let entry = source[start..end].trim();
                let member = entry
                    .strip_prefix('?')
                    .filter(|rest| {
                        rest.is_empty() || rest.chars().next().is_some_and(char::is_whitespace)
                    ...
                    .map(str::trim_start)
                    .unwrap_or(entry);
                let colon = flow_top_level_mapping_colon(member, 0, member.len());
                let member = colon.map_or(member, |colon| &member[..colon]);
                (!member.trim().is_empty()).then(|| member.trim_end().to_owned())
            })
    ...
    members
        ...
        .map(|(member_index, (line_index, start, member_indent))| {
            ...
            if let Some(explicit_value_start) = spans
                .iter()
                .skip(*line_index + 1)
...
fn set_member_collection_lexical_source(source: &str) -> String {
    let end = line_spans(source)
        ...
        .find_map(|line| {
            ...
            trimmed
                ...
                .filter(|rest| semantic_null_set_member(rest.trim_start()))
                .map(|_| line.start)
        })
        .unwrap_or(source.len());
    collection_lexical_source(&source[..end])
}
...
fn restore_deferred_map_member_source(member_source: &str, member: &mut Value) {
        ...
        && tagged.get("tag").and_then(Value::as_str) == Some("tag:yaml.org,2002:map")
    {
        tagged.insert(
            "source".to_owned(),
            Value::String(set_member_collection_lexical_source(
                &member_source[source_start..],
            )),
        );
    }
}
...
fn restore_collection_set_member_provenance(
            ...
            })
    {
        ...
        let lexical = set_member_collection_lexical_source(&member_source[nested_start..]);
            ...
            spans
                ...
                .find_map(|candidate| {
                    ...
                    Some({
                        ...
                        if let Some(value) = deferred_remainder.strip_prefix(':')
                            && (value.is_empty()
                                || value.chars().next().is_some_and(char::is_whitespace))
                            && semantic_null_deferred_set_property(value.trim_start())
                        {
                            let terminal = if value.trim().is_empty() {
                                candidate.content_end
                            } else {
                                candidate.end
                            };
                            return Some(source[base_start..terminal].to_owned());
                        }
...
impl FlowTagScanner<'_, '_> {
    ...
    fn parse_value(&mut self, path: &mut Vec<String>) {
        ...
        if let Some(tag_name) = tag_name {
            ...
            let flow_value_end = matches!(self.peek(), Some('{' | '['))
                .then(|| flow_value_end(self.source, value_start));
            let previous_limit = self.limit;
            if let Some(flow_value_end) = flow_value_end {
                self.limit = self.limit.min(flow_value_end);
            }
            ...
            if let Some(flow_value_end) = flow_value_end {
                self.cursor = self.cursor.max(flow_value_end);
            }
    ...
    fn parse_mapping(&mut self, path: &mut Vec<String>) {
        ...
        loop {
            ...
            let Some(colon) = flow_mapping_colon(self.source, key_start, self.limit) else {
                return;
            };
    ...
    fn parse_sequence(&mut self, path: &mut Vec<String>) {
        ...
        loop {
            ...
            path.push(index.to_string());
            let (item_end, _) = flow_mapping_value_end(self.source, self.cursor, self.limit);
    ...
    fn remaining(&self) -> &str {
        &self.source[self.cursor..self.limit]
    }
...
fn internal_standard_tag(tag: &str) -> Option<&'static str> {
    Some(match tag {
        ...
        "!oseq" => "tag:yaml.org,2002:seq",
        "!omap-value" => "tag:yaml.org,2002:map",
        "!oset-value" => "tag:yaml.org,2002:set",
        "!oomap" => "tag:yaml.org,2002:omap",
        "!opairs" => "tag:yaml.org,2002:pairs",
        _ => return None,
    })
}
...
fn top_level_field_ranges(text: &str, start: usize, end: usize) -> Map<String, Value> {
    let normalized_text = normalize_permitted_yaml_tabs(text);
    let scan_text = normalized_text.as_str();
    let source = &scan_text[start..end];
    if let Some(flow_start) = root_flow_mapping_start(source) {
        return flow_field_ranges(scan_text, start + flow_start, end);
    }
    ...
    for (line_index, line) in spans.iter().enumerate() {
        ...
        if let Some(_explicit_key) = trimmed.strip_prefix('?') {
            ...
            let inline_end = if scalar_has_indented_continuation(
                ...
                multiline_quoted_value_end(scan_text, value_start, end, value_source)
            } else {
                inline_field_value_end(scan_text, value_start, start + value_line.content_end)
            };
            starts.push((
                name,
                field_start,
        ...
        let field_start = if plain_key_source.is_empty() && name.is_empty() && key_tag.is_some() {
            line_start + colon
        } else {
            line_start
                + key_source
        ...
        let inline_end = if scalar_has_indented_continuation(
            ...
            inline_field_value_end(scan_text, value_start, start + line.content_end)
        };
        starts.push((
            name,
            field_start,
        ...
        starts.iter().enumerate()
    {
        ...
        let calculated_field_end = inline_end.unwrap_or_else(|| {
        ...
        let field_end = deferred_property_field_end(
            scan_text,
            start + spans[*line_index].end,
            next_start,
            value_source,
            calculated_field_end,
        )
        .unwrap_or(calculated_field_end);
        fields.insert(
            name.clone(),
...
fn deferred_property_field_end(
    text: &str,
    content_start: usize,
    boundary: usize,
    value_source: &str,
    calculated_end: usize,
) -> Option<usize> {
    ...
    if !value_source.is_empty() {
        let (remainder, anchor, tag) = split_node_properties(value_source);
        if !(remainder.is_empty() || remainder.starts_with('#'))
            || (anchor.is_none() && tag.is_none())
        {
            return None;
        }
        saw_property = true;
        saw_set_property = tag == Some("set");
    }
    for line in line_spans(&text[content_start..boundary]) {
        ...
        let (property_remainder, property_anchor, property_tag) = split_node_properties(trimmed);
        if (property_remainder.is_empty() || property_remainder.starts_with('#'))
            && (property_anchor.is_some() || property_tag.is_some())
        {
            saw_property = true;
            if property_tag == Some("set") {
        ...
        if saw_set_property
            && let Some(value) = trimmed.strip_prefix(':').filter(|value| {
                value.is_empty() || value.chars().next().is_some_and(char::is_whitespace)
            })
        {
            if !semantic_null_deferred_set_property(value.trim_start()) {
                return None;
            }
            return Some(
                content_start
        ...
        if matches!(property_remainder.chars().next(), Some('{' | '['))
            || trimmed.contains('\u{0085}')
        {
            return None;
        }
        let block_node = mapping_key_colon(trimmed).is_some()
            || trimmed == "-"
            || trimmed.starts_with("- ")
            || block_scalar_indicator(trimmed).is_some();
        if !block_node {
            return None;
        }
        return line_spans(&text[content_start..boundary])
            .into_iter()
            .find_map(|terminal| {
                let content_end = content_start + terminal.content_end;
                (content_end == calculated_end && terminal.end > terminal.content_end)
                    .then_some(content_start + terminal.end)
            });
    }
    None
}
...
fn block_field_value_end(
    ...
    let first_continuation_is_collection = continued_lines.first().is_some_and(|(_, trimmed)| {
        trimmed.strip_prefix('?').is_some_and(|remainder| {
            remainder.is_empty() || remainder.chars().next().is_some_and(char::is_whitespace)
        }) || *trimmed == "-"
            || trimmed.starts_with("- ")
    });
        ...
        && let Some((content_end, _)) = last_nonblank
    {
        ...
        let terminal_empty_explicit_null = terminal
            .strip_prefix(':')
            .is_some_and(|rest| rest.trim().is_empty());
        if !terminal_in_block_scalar
            && (terminal == "?" || terminal_member_property || terminal_empty_explicit_null)
        {
            return content_end;
        }
    }
...
fn markdown_links(
    ...
    let definition_spans = markdown_definition_inspection(body)?;
    let (all_reference_definitions, reference_definitions) = markdown_reference_definitions(body);
    let parser_body = markdown_reference_parser_body(body, &all_reference_definitions)?;
    ...
    for (event, range) in parser {
        match event {
            Event::Start(Tag::Link {
                ...
                if stack
                    .iter()
                    .any(|(kind, _, _, _)| *kind == MarkdownMediaKind::Image)
                {
                    stack.push((
                        MarkdownMediaKind::IgnoredLink,
                        String::new(),
                        String::new(),
                        range,
                    ));
                ...
                if definition_spans.iter().any(|definition| {
                    definition.start <= range.start && range.start < definition.end
                }) {
                    stack.push((
                        MarkdownMediaKind::IgnoredLink,
                        String::new(),
                ...
                let raw_destination = dest_url.into_string();
                if link_type == LinkType::Email
                    && !markdown_micromark_email_autolink(&raw_destination)
                {
                    stack.push((
                        MarkdownMediaKind::IgnoredLink,
                        String::new(),
            ...
            Event::End(end @ (pulldown_cmark::TagEnd::Link | pulldown_cmark::TagEnd::Image)) => {
                if let Some((kind, label, target, start_range)) = stack.pop() {
                    debug_assert!(match end {
                        pulldown_cmark::TagEnd::Link => matches!(
                            kind,
                            MarkdownMediaKind::Link | MarkdownMediaKind::IgnoredLink
                        ),
                        pulldown_cmark::TagEnd::Image => kind == MarkdownMediaKind::Image,
                        _ => false,
                    });
                    if kind == MarkdownMediaKind::IgnoredLink {
                        continue;
                    }
    ...
    result.sort_by(|left, right| {
        left.range
            ...
            .then_with(|| right.range.end.offset.cmp(&left.range.end.offset))
    });
    Ok(result)
}
...
fn markdown_reference_parser_body(
    body: &str,
    ...
    for definition in all_definitions {
        ...
        let Some(colon) = label_end
            ...
            return Err(markdown_reference_bounding_failure());
        };
    ...
    String::from_utf8(inspection_body).map_err(|_| markdown_reference_bounding_failure())
}

fn markdown_reference_definition_source_label<'a>(
    body: &'a str,
    span: &std::ops::Range<usize>,
) -> Option<&'a str> {
...
fn markdown_reference_bounding_failure() -> String {
    "Markdown reference definitions could not be bounded safely before parsing. Reduce or split the document, then retry."
        .to_owned()
}
...
fn markdown_definition_inspection(body: &str) -> Result<Vec<std::ops::Range<usize>>, String> {
    let definition_parser = Parser::new_ext(body, Options::ENABLE_STRIKETHROUGH);
    let parsed_definitions = definition_parser
        .reference_definitions()
        .iter()
        .map(|(label, definition)| (label.to_string(), definition))
        .collect::<Vec<_>>();
    let spans = line_spans(body);
    let mut definition_count = parsed_definitions.len();
    ...
    let mut definition_spans = Vec::with_capacity(parsed_definitions.len());
    for (_, definition) in &parsed_definitions {
        let Some(label) = markdown_reference_definition_source_label(body, &definition.span) else {
            return Err(markdown_reference_bounding_failure());
        };
        if let Some(message) = bounded_link_text_failure(
            label,
    ...
    while line_index < spans.len() {
        ...
        if let Some((parsed_label, definition)) =
            parsed_definitions.iter().find(|(_, definition)| {
                span.start <= definition.span.start && definition.span.start < span.end
            })
        {
            seen_definition_labels.insert(normalize_reference_identifier(parsed_label));
        ...
        let Some(closing) = closing else {
            if let Some((raw_label, destination, consumed_lines)) =
                markdown_multiline_duplicate_definition_candidate(
                    body,
                    &spans,
                    line_index,
                    trimmed,
                    &seen_definition_labels,
                    markdown_container_signature(source_line),
                    markdown_innermost_list_content_indent(source_line),
                )
            {
                definition_count += 1;
                if definition_count > MAX_MARKDOWN_DEFINITIONS_PER_DOCUMENT {
                ...
                let definition_end_line = line_index
                    .saturating_add(consumed_lines)
                    .min(spans.len().saturating_sub(1));
                definition_spans.push(span.start..spans[definition_end_line].end);
...
fn markdown_multiline_duplicate_definition_candidate(
    body: &str,
    spans: &[LineSpan],
    line_index: usize,
    first_line: &str,
    seen_labels: &BTreeSet<String>,
    container_signature: Vec<u8>,
    list_content_indent: Option<usize>,
) -> Option<(String, String, usize)> {
    let mut fragment = String::new();
    fragment.push_str(first_line);
    fragment.push('\n');
    for (continuation_index, span) in spans.iter().enumerate().skip(line_index + 1) {
        let source = &body[span.start..span.content_end];
        if !markdown_duplicate_continuation_matches(
            source,
            &container_signature,
            list_content_indent,
        ) {
        ...
        let continuation = markdown_container_content(source);
        if continuation.trim().is_empty() {
            break;
        }
        fragment.push_str(continuation);
        fragment.push('\n');
        let Some((raw_label, normalized_label)) =
            markdown_definition_candidate_label_and_source(&fragment)
        else {
            if !markdown_definition_label_continues(&fragment) {
                return None;
            }
            continue;
        };
        ...
        if let Some((destination, consumed_lines)) =
            markdown_definition_from_fragment(&fragment, &normalized_label)
        {
            let next_line_starts_title = spans.get(continuation_index + 1).is_some_and(|span| {
                let source = &body[span.start..span.content_end];
                markdown_duplicate_continuation_matches(
                    source,
                    &container_signature,
                    list_content_indent,
                ) && markdown_container_content(source)
                    .trim_start()
                    .starts_with(['"', '\'', '('])
            });
            if next_line_starts_title {
                continue;
            }
            return Some((raw_label, destination, consumed_lines));
        }
        if continuation_index + 1 == spans.len() {
            break;
        }
    }
    None
...
fn markdown_duplicate_definition_candidate(
    ...
    fragment.push_str(first_line);
    fragment.push('\n');
    let first_definition = markdown_definition_from_fragment(&fragment, expected_label);
    let first_line_terminator = first_definition
        .is_none()
        .then(|| markdown_definition_title_terminator(first_line))
        .flatten();
    let next_line_terminator = spans.get(line_index + 1).and_then(|span| {
        let source = &body[span.start..span.content_end];
        markdown_duplicate_continuation_matches(source, &container_signature, list_content_indent)
            .then(|| {
                markdown_container_content(source)
                    .trim_start()
                    .as_bytes()
            ...
            .flatten()
            .and_then(markdown_title_terminator)
    });
    let (terminator, title_starts_on_next_line) = if let Some(terminator) = first_line_terminator {
        (terminator, false)
    } else if let Some(terminator) = next_line_terminator {
        (terminator, true)
    } else {
    ...
    for (continuation_index, span) in spans.iter().enumerate().skip(line_index + 1) {
        ...
        fragment.push_str(continuation);
        fragment.push('\n');
        let title_content = if title_starts_on_next_line && continuation_index == line_index + 1 {
            continuation.trim_start().get(1..).unwrap_or_default()
        } else {
            continuation
        };
        if markdown_contains_unescaped_byte(title_content, terminator) {
            return markdown_definition_from_fragment(&fragment, expected_label)
                .or(first_definition);
        }
    }
    first_definition
...
fn markdown_definition_title_terminator(line: &str) -> Option<u8> {
    let tail = line.split_once(':')?.1.trim_start();
    let after_destination = if let Some(enclosed) = tail.strip_prefix('<') {
        let end = markdown_unescaped_byte_index(enclosed, b'>')?;
        &enclosed[end + 1..]
    } else {
        let end = tail.find(char::is_whitespace).unwrap_or(tail.len());
        &tail[end..]
    };
    markdown_title_terminator(*after_destination.trim_start().as_bytes().first()?)
}

fn markdown_title_terminator(opener: u8) -> Option<u8> {
    match opener {
        b'"' | b'\'' => Some(opener),
        b'(' => Some(b')'),
        _ => None,
...
fn markdown_contains_unescaped_byte(value: &str, expected: u8) -> bool {
    markdown_unescaped_byte_index(value, expected).is_some()
}

fn markdown_unescaped_byte_index(value: &str, expected: u8) -> Option<usize> {
    let mut escaped = false;
    for (index, byte) in value.bytes().enumerate() {
        if byte == expected && !escaped {
            return Some(index);
        }
...
fn markdown_definition_candidate_label(line: &str) -> Option<String> {
    markdown_definition_candidate_label_and_source(line).map(|(_, normalized)| normalized)
}

fn markdown_definition_label_continues(line: &str) -> bool {
    let Some(after_open) = line.strip_prefix('[') else {
        return false;
    };
    let mut escaped = false;
    for byte in after_open.bytes() {
        if byte == b']' && !escaped {
            return false;
        }
        escaped = byte == b'\\' && !escaped;
...
fn markdown_definition_candidate_label_and_source(line: &str) -> Option<(String, String)> {
    let after_open = line.strip_prefix('[')?;
    let mut escaped = false;
    for (offset, character) in after_open.char_indices() {
        if character == ']' && !escaped {
            let remainder = &after_open[offset + character.len_utf8()..];
            return remainder.starts_with(':').then(|| {
                let source = after_open[..offset].to_owned();
                let normalized = normalize_reference_identifier(&source);
                (source, normalized)
            });
        }
        escaped = character == '\\' && !escaped;
...
fn markdown_definition_from_fragment(
    fragment: &str,
    expected_label: &str,
) -> Option<(String, usize)> {
    ...
    parser
        .reference_definitions()
        .iter()
        .find_map(|(label, definition)| {
            (definition.span.start == 0 && normalize_reference_identifier(label) == expected_label)
                .then(|| {
                    (
...
fn markdown_work_inspection(body: &str) -> MarkdownWorkInspection {
    ...
    let mut inline_html_terminator: Option<(&'static str, Vec<u8>)> = None;
    ...
    let definition_spans = definition_parser
        ...
        .collect::<Vec<_>>();
    let definition_labels = definition_parser
        .reference_definitions()
        .iter()
        .map(|(label, _)| normalize_reference_identifier(label))
        .collect::<BTreeSet<_>>();
    let mut duplicate_definition_through_line = None;
    for (line_index, span) in spans.iter().enumerate() {
        let source_line = &body[span.start..span.content_end];
        let (line, container_depth) = markdown_container_content_and_depth(source_line);
        ...
        if duplicate_definition_through_line.is_some_and(|end| line_index <= end) {
            paragraph_depth = None;
            continue;
        }
        if let Some(label) = markdown_definition_candidate_label(trimmed)
            && definition_labels.contains(&label)
            && let Some((_, consumed_lines)) = markdown_duplicate_definition_candidate(
                body,
                &spans,
                line_index,
                trimmed,
        ...
        {
            duplicate_definition_through_line = Some(line_index + consumed_lines);
            paragraph_depth = None;
            continue;
        }
        if markdown_definition_label_continues(trimmed)
            && let Some((_, _, consumed_lines)) = markdown_multiline_duplicate_definition_candidate(
                body,
                &spans,
                line_index,
                trimmed,
        ...
        let mut index = 0usize;
        if let Some((terminator, signature)) = inline_html_terminator.take()
            && markdown_inline_html_continuation_matches(source_line, &signature)
        {
            if let Some(end) = trimmed.find(terminator) {
                index = end + terminator.len();
            } else {
                inline_html_terminator = Some((terminator, signature));
        ...
        while index < bytes.len() {
            ...
            if bytes[index] == b'<' {
                if let Some(terminator) =
                    markdown_multiline_inline_html_terminator(&trimmed[index..])
                    && markdown_inline_html_terminator_ahead(
                        body,
                        &spans,
                        line_index,
                        &trimmed[index..],
                ...
                {
                    if let Some(end) = trimmed[index..].find(terminator) {
                        index += end + terminator.len();
                    } else {
                        inline_html_terminator =
                            Some((terminator, markdown_container_signature(source_line)));
                        break;
                    }
                    continue;
                }
                if let Some(end) = markdown_inline_angle_end(trimmed, index) {
                    index = end;
                    continue;
                }
            }
...
fn markdown_multiline_inline_html_terminator(value: &str) -> Option<&'static str> {
...
fn markdown_inline_html_terminator_ahead(
    body: &str,
    spans: &[LineSpan],
    line_index: usize,
    current_line: &str,
    terminator: &str,
    container_signature: &[u8],
) -> bool {
    current_line.contains(terminator)
        || spans
            .iter()
            .skip(line_index + 1)
            .take_while(|span| {
                let line = &body[span.start..span.content_end];
                !markdown_container_content(line).trim().is_empty()
                    && markdown_inline_html_continuation_matches(line, container_signature)
            })
            .any(|span| {
                markdown_container_content(&body[span.start..span.content_end]).contains(terminator)
            })
}

fn markdown_micromark_email_autolink(value: &str) -> bool {
    let Some((local, domain)) = value.split_once('@') else {
        return false;
    };
    if local.is_empty() || !local.bytes().all(markdown_micromark_email_atext) {
        return false;
    }
...
fn markdown_micromark_email_atext(byte: u8) -> bool {
    matches!(
        byte,
...
fn markdown_inline_angle_end(line: &str, index: usize) -> Option<usize> {
    ...
    let candidate = &line[index..cursor + 1];
    let autolink = !content.chars().any(char::is_whitespace)
        ...
        }) || markdown_micromark_email_autolink(content));
    let tag = markdown_complete_html_tag(candidate);
    let cdata = candidate.starts_with("<![CDATA[") && candidate.ends_with("]]>");
    let declaration = candidate
        .strip_prefix("<!")
        .and_then(|remainder| remainder.as_bytes().first())
        .is_some_and(u8::is_ascii_alphabetic);
    let special_html = (candidate.starts_with("<!--") && candidate.ends_with("-->"))
...
fn markdown_container_signature(mut line: &str) -> Vec<u8> {
...
fn markdown_duplicate_continuation_matches(
    line: &str,
    expected: &[u8],
    list_content_indent: Option<usize>,
) -> bool {
    let actual = markdown_container_signature(line);
    ...
    actual == expected[..expected.len() - 1]
        && markdown_indentation_columns(markdown_container_content(line))
            >= list_content_indent.unwrap_or(usize::MAX)
}

fn markdown_inline_html_continuation_matches(line: &str, expected: &[u8]) -> bool {
    let actual = markdown_container_signature(line);
    if actual == expected {
        return expected.last() != Some(&b'-');
    }
    actual.len() < expected.len() && expected.starts_with(&actual)
}

fn markdown_innermost_list_content_indent(mut line: &str) -> Option<usize> {
    let mut result = None;
    loop {
        let leading = line
            .chars()
        ...
        if let Some(after_marker) = markdown_list_item_content(candidate) {
            let consumed = &candidate[..candidate.len() - after_marker.len()];
            result = if let Some(marker) = consumed.strip_suffix('\t') {
                (matches!(marker, "-" | "+" | "*" | "1." | "1)")).then_some(0)
            } else {
                Some(leading + consumed.len())
            };
            line = after_marker;
            continue;
        }
...
mod tests {
    ...
    #[test]
    fn finds_duplicate_properties_split_across_sequence_lines() {
        let source = "x:\n  - !!str\n    !!int\n    text\n";
        assert_eq!(
            multiple_block_node_property_range(source, NodePropertyKind::Tag),
            Some((17, 22))
        );
    }
    ...
    #[test]
    fn adjusts_cached_radix_decimals_across_carry_boundaries() {
        assert_eq!(decimal_with_signed_delta("999", 2).as_deref(), Some("1001"));
        assert_eq!(
            decimal_with_signed_delta("1000", -1).as_deref(),
            Some("999")
        );
        assert_eq!(
            decimal_with_signed_delta("100000", -999).as_deref(),
            Some("99001")
        );
    }

    #[test]
    fn direct_set_member_source_stops_before_explicit_null_value() {
        let source = "? !!map\n  k: v\n:\n? next\n";
        assert_eq!(
            direct_standard_set_member_sources(source, 2),
            vec!["!!map\n  k: v\n", "next\n"]
        );
    }
