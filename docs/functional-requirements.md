...
| FR-028 | One selected-bundle load MUST have a deterministic provider-resource envelope and bounded physical concurrency. | At most 2,000 Markdown documents, 2 MiB per document, 32 MiB total reported and actual Markdown bytes, sixteen concurrent provider calls, 64 path segments, 32 MiB cumulative retained traversal identity, and 128 retained failures are admitted. Provider paths are at most 4,096 code units and UTF-8 bytes; serialized source URIs are at most 16 KiB so a fully percent-expanded 4 KiB path remains representable. Exact limits pass. A one-over document becomes one document-scoped identity-only failure without suppressing readable siblings; malformed UTF-16 in a bundle-root URI fails the complete bundle, while malformed UTF-16 in a document URI, provider path, content identity, source text, or retained provider-failure detail is rejected or sanitized at document scope without suppressing readable siblings or crossing the Wasm JSON boundary. Every materialized provider byte remains charged to the actual aggregate even when later parent revalidation discards it; and a count, aggregate, root, depth, or traversal-identity overflow fails the refresh. Every already-started provider call is drained before a canceled or failed generation settles. |
...
| FR-031 | Conformance errors MUST include missing or invalid YAML frontmatter, missing or empty concept `type`, invalid reserved-document structure, and unconsumable UTF-8 input. Bundle-root `index.md` frontmatter MUST contain no key other than the optional `okf_version` declaration. | A fixture for each case is reported as a conformance error. An extra root-index key points to that exact field while the parsed value remains retained for source-preserving repair. |
...
| FR-033 | Unknown concept types and additional concept frontmatter fields MUST NOT produce errors or warnings solely because they are unknown. This does not relax reserved-document structure. | A valid custom-type fixture with custom fields has no unknown-schema finding; an extra bundle-root `index.md` frontmatter key remains a reserved-document conformance error. |
...
| FR-078 | Generated instructions SHOULD prefer the native `okf` CLI for local-filesystem validation, new-concept and managed-index planning, and guarded create publication when it is available, but MUST remain usable without it. | `AGENTS.md` keeps one concise optional-CLI rule; the Skill provides `validate`, `new --check`, and `index --check` examples, limits `--apply` to a reviewed single create in an existing root or a reviewed all-create plan for an absent root, and directs existing-root multi-create plans, existing-file updates, provider-backed bundles, and CLI-free environments to split commands, editor commands, or document rules. |
| FR-079 | `OKF: Migrate Bundle to v0.2` MUST be explicit, deterministic, previewed, and source-preserving. | A caller-supplied actor converts eligible single-line, unanchored legacy timestamps; simple URL citation bullets add sources without deleting Citations or treating indented code as citations; ambiguous, anchored, and multiline cases are reported for manual follow-up; unknown fields and body text survive; the root declaration is ordered last within the same guarded proposal; and a second core-planning run is empty. The extension applies only the approved guarded proposal. The native CLI requires `--check`, reports the complete plan without writing, and does not expose migration `--apply` while generation-CAS replacement is unavailable. A host notification that remains pending or rejects neither delays the extension preview nor retains its write-command lease. |

### 8. Common file and workspace behavior

| ID | Requirement | Acceptance condition |
| --- | --- | --- |
| FR-080 | All core commands MUST operate without an account or network connection. | Initialize, create, validate, index, migration, graph, and agent-template acceptance flows pass with network access disabled. |
...
| FR-083 | File watching MUST cover create, change, delete, and rename events and debounce related bursts. | One logical edit burst results in current diagnostics and graph state without persistent duplicate processing. Windows root/event drive-letter casing differences must not discard events; sibling roots and different drives remain excluded. |
...
| ID | Requirement | Acceptance condition |
| --- | --- | --- |
| FR-100 | Production extension semantics and the native CLI MUST use the same Rust `okf-core` implementation for parsing, validation, graph construction, indexes, bundle/concept templates, migration, and agent templates. | Canonical fixtures, generated bytes, and migration plans pass parity tests through native Rust and the packaged Wasm ABI; the production activation loads the packaged Wasm core once and does not silently fall back after a load, ABI, trap, or response failure. |
...
| FR-102 | The native `okf` CLI MUST provide `init`, `new`, `validate`, `index`, `graph`, `agent`, `migrate`, and `version`. | CLI integration tests exercise each command family and machine-readable output uses a `schemaVersion: 1` envelope. |
...
| FR-104 | CLI writes MUST remain contained and recoverable at the operation boundary. | Paths cannot escape the selected root; planning retains an open identity-bound handle for the existing root or deepest existing ancestor through apply; and symbolic-link traversal and root replacement are rejected. The root `index.md` version gate is bound to the planned handle, identity, and exact bytes (or planned absence) and is revalidated before mutation. One create in an existing root is published without overwrite; an existing-root plan with multiple creates fails before any write because the supported filesystems do not provide a complete-plan transaction. All missing-root processes sharing one retained ancestor—including case/Unicode aliases—contend for one platform reservation and require no ancestor-wide scan. Unix atomically creates an anchor-identity-derived, dot-plus-decimal coordination file, then uses one 128-bit OS-random staging directory. Windows atomically creates and obtains the strict staging-directory handle at the fixed `00000000.000` component; because that component is itself an ASCII-numeric 8.3 name, it has no distinct generated short-name alias, and an exact or trailing-dot/space-equivalent requested-root match is rejected before reservation. Unix arms best-effort coordination cleanup immediately after reservation, including randomness failure; an observed identity mismatch is not removed, and cleanup failure retains owned residue and is reported. Every staged leaf is bound to its created identity and exact bytes: Windows retains deny-write-and-delete-sharing leaf and descendant-directory handles through post-hook validation, while Unix reopens handle-relative paths and revalidates identity and bytes after the publication hook. Windows releases descendant handles immediately before its ancestor-root rename because the platform forbids that rename while descendants are open. Unix then revalidates the complete anchor pathname identity; staging existence and cleanup remain relative to the retained ancestor handle. The retained staging identity is revalidated after the publication hook, and the complete absent root subtree is published with one same-parent no-replace rename. Exact source/destination name equality is rejected before reservation; unrelated missing-root publishes below the same anchor may fail closed and can be retried; a failed/interrupted Unix publisher leaves at most one coordination file and one staging tree. An observed substitution after Unix identity capture or a destination collision publishes no generated leaf; an entry already present at reservation or observed with a mismatched identity is not removed; success leaves no staging or coordination entry. POSIX exposes neither create-directory-and-return-handle, source-generation CAS, nor handle-bound unlink CAS, so non-cooperating same-UID mutation in the documented `mkdirat`-to-`openat`, staged-leaf/staging/anchor identity-check-to-rename, and coordination identity-check-to-`unlinkat` windows is outside the guarantee. Windows also lacks a descendant-source-generation CAS: releasing required descendant handles leaves a final release-to-root-rename window outside the guarantee for a non-cooperating process with mutation rights, while retained handles deny direct writes and replacements through final validation. On Windows, an existing-root create additionally requires its direct parent to exist at planning so its handle and identity can be retained through apply. Because supported filesystems do not expose the complete generation-CAS and metadata-preserving primitive required by this contract, a plan containing any existing-file update likewise fails before root creation or any proposal write, while `--check` still reports the proposed change. |
...
QR-001 remains a hypothesis until a timed first-use study is retained. The current-candidate
schema-v3 headed VS Code `1.129.1` report measures QR-002 at `873 ms` p95 across 20 samples and
records `d3` for QR-003, bound to the exact `0.3.0` production and harness inputs. The older VS Code
`1.127.0` capture is historical and no longer passes the current strict evaluator. See
[Performance evidence](performance-evidence.md). QR-004 is evidenced for the exact `0.3.0`
candidate by the documented VSCodium hosted lanes; QR-005 remains bounded by its retained manual
and component evidence rather than a complete packaged keyboard-only acceptance flow. No release
documentation may present candidate-specific or partially evidenced results as generally
achieved.
...
| OQ-005 | Seven core `okfWorkbench.*` workflow command IDs, editor Webview graph, and a fixed 250 ms debounce. Presentation-only sidebar command IDs remain outside the read/write acceptance catalog and delegate to those core workflows. | Extension manifest, settings |
