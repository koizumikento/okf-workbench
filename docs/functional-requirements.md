...
| FR-079 | `OKF: Migrate Bundle to v0.2` MUST be explicit, deterministic, previewed, and source-preserving. | A caller-supplied actor converts eligible legacy timestamps; simple URL citation bullets add sources without deleting Citations; ambiguous cases are reported for manual follow-up; unknown fields and body text survive; the root declaration changes within the same guarded proposal; and a second run is empty. The extension applies only the approved guarded proposal; the native CLI reports the complete plan with `--check` and fails closed on `--apply` while generation-CAS replacement is unavailable. |
