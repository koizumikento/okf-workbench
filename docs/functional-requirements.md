...
| FR-078 | Generated instructions SHOULD prefer the native `okf` CLI for local-filesystem validation, new-concept and managed-index planning, and create-only publication when it is available, but MUST remain usable without it. | `AGENTS.md` keeps one concise optional-CLI rule; the Skill provides `validate`, `new --check`, and `index --check` examples, limits `--apply` to reviewed all-create plans, and directs existing-file updates, provider-backed bundles, and CLI-free environments to editor commands and document rules. |
...
| FR-104 | CLI writes MUST remain contained and recoverable at the operation boundary. | Paths cannot escape the selected root; the plan retains the filesystem identity of the root or deepest existing ancestor through apply; and symbolic-link traversal and root replacement are rejected. Create-only targets are published without overwrite: Unix uses no persistent staging pathname, while Windows uses one fixed reservation name per target directory, never removes an existing reservation by pathname, attempts handle-bound cleanup after handled failure, and reports a cleanup failure while residue remains bounded to that reservation. Because supported filesystems do not expose the complete generation-CAS and metadata-preserving primitive required by this contract, a plan containing any existing-file update fails before root creation or any proposal write, while `--check` still reports the proposed change. |
