...
The repository-owned package wrapper sets a fixed `SOURCE_DATE_EPOCH`, which makes pinned `vsce` sort ZIP entries lexicographically, and then normalizes every local-header and central-directory DOS timestamp to `1980-01-01 00:00:00`. This removes both asynchronous file-discovery ordering and clock-dependent bytes while preserving entry contents, CRCs, compression, attributes, and comments. The normalizer fails closed for ZIP64, split archives, and timestamp-bearing ZIP extra fields. CI invokes that same wrapper a second time against the unchanged build and requires byte equality with the candidate before retaining it. Byte identity across different operating systems still requires identical tracked inputs, file modes, line endings, and pinned tool versions; entry and timestamp normalization alone are not evidence of cross-platform reproducibility.
