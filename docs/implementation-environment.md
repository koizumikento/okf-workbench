...
The repository-owned package command normalizes every local-header and central-directory ZIP DOS timestamp to `1980-01-01 00:00:00` immediately after `vsce` runs. This removes the observed clock-dependent bytes while preserving entry contents, CRCs, compression, ordering, attributes, and comments. The normalizer fails closed for ZIP64, split archives, and timestamp-bearing ZIP extra fields. CI invokes the pinned `vsce` a second time against the unchanged build, normalizes the temporary result, and requires byte equality with the candidate before retaining it. Byte identity across different operating systems still requires identical tracked inputs, file modes, line endings, and pinned tool versions; timestamp normalization alone is not evidence of cross-platform reproducibility.
