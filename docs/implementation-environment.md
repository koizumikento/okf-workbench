...
The native CLI uses the locked `rustix` filesystem API on Unix to anchor reads and mutations to
opened bundle and parent-directory descriptors, traverse with no-follow semantics, fully stage new
bytes in the target directory, and publish with an atomic no-replace or exchange operation. The
exchange path verifies the displaced file through the anchored directory and rolls back when it is
not the exact content and identity approved during planning. On Linux, replacements copy the
existing owner/group, POSIX access ACL, SELinux label, `user.*` extended attributes, and mode through
file descriptors before publication; content-sensitive security attributes such as capabilities,
IMA, and EVM are intentionally not transferred to changed bytes. On macOS the locked `libc` binding
uses `fcopyfile(COPYFILE_METADATA)` to retain mode, owner/group, ACLs, and extended attributes before
the exchange. POSIX has no portable compare-and-unlink operation, so the CLI retains the verified
displaced file—or a staged file from a failed publication—under its hidden generated recovery name
instead of deleting a pathname after a separate identity check that another writer could race.

On Windows the locked `windows-sys` filesystem binding opens every existing root-to-parent
directory without following reparse points and retains handles that do not share delete access.
Reads use the verified file handle. Creates and updates are fully staged in the same directory and
published relative to the retained parent handle with `FileRenameInfoEx`; an existing target remains
open without shared write or delete access until the single POSIX-semantics replacement call. A
junction/reparse-point component, unavailable atomic operation, metadata-copy failure, or temporary
write failure therefore fails closed before generated bytes are published. These filesystem crates
