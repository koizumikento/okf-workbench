...
export function renderAgentSkill(bundlePath: BundleDirectoryInput): OperationResult<string> {
  ...
  const lines = [
    ...
    '```',
    '',
    'Inspect every reported path and change before rerunning an all-create write command with `--apply` instead of `--check`. A plan containing an existing-file update fails closed; use the editor workflow or careful manual editing for that update. Edit existing concept Markdown directly while preserving unknown frontmatter.',
    '',
    '## Concept documents',
    '',
