...
export function renderAgentSkill(bundlePath: BundleDirectoryInput): OperationResult<string> {
  ...
  const lines = [
    ...
    '```',
    '',
    'Inspect every reported path and change before replacing `--check` with `--apply`. Apply only one new file in an existing bundle, or a complete all-create plan whose bundle root is still absent. Multiple creates in an existing root and every existing-file update fail closed; split new files into single-create commands or use the editor workflow or careful manual editing. Edit existing concept Markdown directly while preserving unknown frontmatter.',
    '',
    '## Concept documents',
    '',
